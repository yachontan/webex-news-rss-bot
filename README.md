...
| **ダイジェスト** | **載せる枠の選択**（天気／各チャンネルのまとめ／時事）と**天気の見せ方**、天気の観測地点（**地名から緯度経度を自動取得**）、**時事ダイジェストの地域バランス**（日本/米国/その他の件数と判定語） |
| **要約AI** | 要約・記事選定に使う AI（**Claude / OpenAI / Gemini**）とモデル名の設定。接続を試して確認できる |
| **自動実行** | 毎朝の実行時刻と曜日を設定。**いまの OS・使う仕組み・登録先を明示**（macOS は launchd、Windows はタスク スケジューラ）。**TCC 保護下や未対応 OS では登録できない旨を表示** |
| **bot とスペース** | bot がどのスペースに入っているかの確認と、**トークンの有効性チェック**。切れていたら**その場で新しいトークンを貼り直せます**（下記） |

### 1スペースあたりの件数を変える / Articles per space

1回に投稿する記事数は既定で**15件**です。超えた分は AI が重要そうな順に絞ります。スペースごとに変えられます。

```yaml
- name: セキュリティ
  webex_space_id: ${WEBEX_SPACE_ID_SECURITY}
  max_items: 8          # 1〜50。書かなければ 15 件
```

セットアップタブのチャンネル設定「詳しい設定」からも指定できます。範囲外や数字でない値を書いた場合は、警告を出して既定の15件で動きます。多くすると1通が長くなり、Webex 側で分割されることがあります。

### ダイジェストの中身を選ぶ / Choosing digest blocks

ダイジェストに載せる枠は、**ダイジェスト**タブで選べます。選んだ順ではなく、下の表の順で投稿されます。

| 枠 | 内容 | 既定 |
|:---|:---|:---:|
| **天気** | 今日・明日の予報（最高/最低気温・降水確率・**平均湿度**）と現在の気温・湿度 | オン |
| **各チャンネルの投稿まとめ** | その日に各スペースへ配信した記事をチャンネル別に集約 | オン |
| **時事ダイジェスト** | `regions.yml` の地域バランス（日本/米国/その他）で選んだ一般ニュース | オフ |

`channels.yml` に直接書くこともできます。

```yaml
- name: 'News Today : 天気とサマリー'
  digest: true
  categories: []
  digest_blocks:        # 載せる枠と順番。書かなければ weather と channels
    - weather
    - channels
  weather_format: table # table=表形式 / list=箇条書き
```

**天気の見せ方**は2種類から選べます。

`table`（既定）は地点を縦に並べた表で、見比べるのに向いています。

```
地点        現在     今日                     明日
─────────────────────────────────────────────────
東京        26° 74%  🌦 27/22° ☔ 90% 💧 83%  ⛅ 29/21° ☔ 19% 💧 73%
函館        23° 69%  🌤 23/18° ☔  0% 💧 79%  🌤 23/18° ☔  0% 💧 80%
```

`list` は従来の箇条書きで、天気の名前（「弱い霧雨」など）まで出ます。

> **なぜコードブロックなのか**: Webex は Markdown の表に対応しておらず、HTML も `h1,h2,h3,ul,ol,u,i,b` とリンクだけです。そのため等幅フォントのコードブロックで桁を揃えて表を描いています。地点名の全角文字と絵文字を表示幅で数えて詰めているので桁は揃いますが、**地点や列が多いとスマホでは横スクロール**になります。読みづらければ `list` に切り替えてください。
...
| **ダイジェスト（天気＋まとめ）** | 毎日の配信後に、**載せる枠を選んで**1通で配信。観測地点は「URL の設定」タブで編集（[詳しくはこちら](#ダイジェストの中身を選ぶ--choosing-digest-blocks)） |
...
### OS ごとの違い / What differs per OS

ウィザードは起動した OS を自動で判定し、「自動実行」タブの先頭に**いまの OS・使う仕組み・登録先**を表示します。手で読み替える必要はありません。

| | macOS | Windows | その他（Linux 等） |
|:---|:---|:---|:---|
| 仕組み | launchd | タスク スケジューラ | ウィザードからは登録できません |
| 登録先 | `~/Library/LaunchAgents/com.webex-news.rssbot.plist` | タスク スケジューラ ライブラリの「rss-bot daily」 | — |
| 呼ばれるファイル | `run_rssbot.sh` | `run_rssbot.bat` | `run_rssbot.sh`（cron 等に自分で登録） |
| 置き場所の制約 | **TCC**（書類・デスクトップ・ダウンロード・iCloud Drive の配下は不可） | **OneDrive 同期フォルダー**の配下は不可 | — |
| スリープ時 | 起動しない。`pmset` で事前に起こす | 既定では復帰しない。タスクのプロパティ →「条件」→「タスクを実行するためにスリープを解除する」 | — |

置き場所が使えない場合や未対応 OS の場合は、**登録ボタン自体を出さず**に理由を表示します。登録できてしまうと、手動実行は動くのに定時実行だけ静かに失敗する状態になり、気づきにくいためです。
